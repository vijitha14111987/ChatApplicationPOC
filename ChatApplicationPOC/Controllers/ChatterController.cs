...
namespace ChatApplicationPOC.Controllers
{
    ...
    public class ChatterController : ControllerBase
    {
        ...
        public ChatterController(ILogger<ChatterController> logger)
        {
            _logger = logger;
        }
        ...
        public async Task<bool> Post([FromBody] Chat message)
        {
            ...
            using (var producer = new ProducerBuilder<Null, string>(config).Build())
            {
                try
                {
                    ...
                    var value = message.SenderName+','+message.Message;
            ...
            using (var consumer = new ConsumerBuilder<Ignore, string>(config).Build())
            {
                ...
                try
                {
                    ...
                    if (masg.Split(',').Length > 1)
                    {
                        ...
                        chat.Message = masg.Split(',')[1];
                    }
                    chat.SenderName = masg.Split(',')[0];
                    c.Add(chat);
